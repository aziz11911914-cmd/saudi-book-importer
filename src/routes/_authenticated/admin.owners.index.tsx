...
function OwnersPage() {
  ...
  const { data: codes, refetch: refetchCodes } = useQuery({ queryKey: ["owner-codes"], queryFn: () => listCodes({ data: { role: "owner" } }) });
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link to="/admin/owners/code" className="rounded-full border border-gold/60 px-4 py-2 text-sm font-semibold text-gold">Create with Invitation Code</Link>
        </div>
      </div>
      ...
      <CodesSection title="Owner Invitation Codes" codes={codes ?? []} onRevoke={async (id) => { await revoke({ data: { id } }); refetchCodes(); }} />
    </div>
  );
}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {codes.map((c: any) => {
              ...
              const label = c.status === "activated" ? "Activated" : c.status === "revoked" ? "Revoked" : expired ? "Expired" : "Pending";
              ...
              return (
                <tr key={c.id} className="border-t border-hairline/40">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{c.status === "activated" ? "—" : new Date(c.expires_at).toLocaleDateString()}</td>
                  <td className="px-4 py-3 text-muted-foreground">{c.used_at ? "Yes" : "No"}</td>
                  <td className="px-4 py-3 text-end">
                    {c.status === "pending" && !expired && (
                      <button onClick={() => onRevoke(c.id)} className="rounded-lg border border-hairline px-3 py-1 text-xs">Revoke</button>
                    )}
                    <button onClick={() => { navigator.clipboard.writeText(c.code).catch(() => {}); }} className="ms-2 rounded-lg border border-hairline px-3 py-1 text-xs">Copy</button>
                  </td>
                </tr>
              );
            })}
